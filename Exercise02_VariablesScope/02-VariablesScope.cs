...
double CalculateRectangleAreas(double width, double height)
{
    ...
    totalSurface += (float)area; // Sumar al total global
    return area;
}

double area1 = CalculateRectangleAreas(double.Parse(args.Length > 0 ? args[0] : "3"),
                                       double.Parse(args.Length > 1 ? args[1] : "4"));

double area2 = CalculateRectangleAreas(double.Parse(args.Length > 2 ? args[2] : "5"),
